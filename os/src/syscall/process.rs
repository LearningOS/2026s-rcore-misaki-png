...
use crate::{
    loader::get_app_data_by_name, mm::{translated_byte_buffer, translated_refmut, translated_str}, task::{
...
fn any_as_u8_slice<T>(buf: &T) -> &[u8]
where T: Sized {
    unsafe {
        core::slice::from_raw_parts(buf as *const T as *const u8, size_of::<T>())
    }
}
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let src = any_as_u8_slice(&time_val);
...
pub fn sys_spawn(path: *const u8) -> isize {
    let new_task = current_task().unwrap().new_for_spawn();
    let token = current_user_token();
    let path = translated_str(token, path);
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        new_task.exec(data);
